...
import { setActivities } from '../../redux/activityWhole';
...
export default function Activities({
    ...
    const activities = useSelector(state => state.activityWhole.activities);
    ...
    const getActivities = (actgrp) => {
        setLoading(true);
        axios.get(`${BASE_URL_ACT}/getActivities?searchText=${searchParam.trim()}&currentPage=${currentPage-1}&zone=${zone.trim()}&licenseType=${licenseType.trim()}&isSpecial=${isSpecial.trim()}&riskClass=${riskClass.trim()}&compRR=${compRR.trim()}&activityGroup=${activityGroup}`,{withCredentials:false})
        .then((res)=>{
        console.log(res)
