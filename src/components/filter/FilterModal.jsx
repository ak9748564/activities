...
export default function FilterModal({zone,setZone}) {
      ...
      <div className="absolute p-10 top-0 left-0 w-full h-screen bg-black/50 flex items-center justify-center transition-all duration-500 ease-out z-10">
            <div className="rounded-md shadow-lg p-4 relative w-full s2:w-[400px] bg-[#F5F5F5]" ref={filterModal ? filterModalRef : filterModalRef}>
              ...
              <div className='flex h-[32px] border-[#ccc] border rounded-md items-center cursor-pointer text-[13px] mt-3'>
                <p className='h-[32px] px-2 leading-[32px] border-r w-[80px] grow-0 shrink-0'>Zone</p>
                <div className='flex items-stretch h-[28px] w-full text-center px-[1px]'>
                  <button className={`w-1/3 h-full leading-[28px] transition-all rounded-sm ${zone === '' ? 'bg-[#24AEE4] text-white shadow-lg' : 'text-black/80 border border-slate-500 shadow-inner'}`} onClick={()=>setZone('')}>All</button>
                  <button className={`w-1/3 h-full leading-[28px] transition-all border-l border-r rounded-sm ${zone === 'Freezone' ? 'bg-[#24AEE4] text-white shadow-lg' : 'text-black/80 border border-slate-500 shadow-inner mx-[1px]'}`} onClick={()=>setZone('Freezone')}>Freezone</button>
                  <button className={`w-1/3 h-full leading-[28px] transition-all rounded-sm ${zone === 'Non-Freezone' ? 'bg-[#24AEE4] text-white shadow-lg' : 'text-black/80 border border-slate-500 shadow-inner'}`} onClick={()=>setZone('Non-Freezone')}>Non-FZ</button>
                </div>         
              </div>
